...
def print_status(fd: int, base: int) -> None:
    ...
    stream_ctrl = read32(fd, base + REG_STREAM_CTRL)
    ...
    print(f"stream_eof        : {bool_word(bool(stream_ctrl & 0x1))}")
    print(f"stream_pingpong   : {bool_word(bool(stream_ctrl & 0x2))}")
    print(f"stream_ptr_error  : {bool_word(bool(stream_status & (1 << 12)))}")
    print(f"stream_pp_status  : {bool_word(bool(stream_status & (1 << 13)))}")
