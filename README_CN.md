...
最新归档的实验 build 已成功生成 bitstream，Vivado 报 `0 Errors`，但时序并未
完全收敛。具体构建说明和时序摘要见：
...
* `TX0` -> `RX1` 和 `TX1` -> `RX0` 的包计数和字节计数路径可用。
* `RX` capture 可以把最近包窗口写入 `DDR4` 并读回。

## 当前限制

* 最新 BRAM-FIFO STREAM build 功能可用，但不是 timing-clean：
